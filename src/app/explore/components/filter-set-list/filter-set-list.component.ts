...
@Component({
  ...
  template: `
    ...
    <section fxLayout="column">
      ...
      <div fxFlex [fxHide]="!visibility" [fxShow]="visibility">
...
export class FilterSetListComponent implements OnChanges {
  ...
  get visibility(): boolean {
    return this.visible && this.filterSets && this.filterSets.length > 0;
  }
  ...
  get showHideIcon(): string {
    return this.visibility ? 'expand_less' : 'expand_more';
  }
  ...
  onStartDateChange(event: MatDatepickerInputEvent<Date>, filterSet: FilterSet) {
    if (event.value) {
  ...
  onEndDateChange(event: MatDatepickerInputEvent<Date>, filterSet: FilterSet) {
    if (event.value) {
