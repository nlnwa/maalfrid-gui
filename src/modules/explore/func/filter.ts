import {AggregateText, Filter, getUTCDate, isSame} from '../../shared/';
...
function timeCondition(time: number, granularity: string): Predicate {
  const t = getUTCDate(fromUnixTime(time));
  return (e: AggregateText) => isSame[granularity](new Date(e.endTime), t);
}
