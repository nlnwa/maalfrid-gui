...
function dateIndexForPeriod(period: Period, time: string): number {
...
export function isSeed(row: Statistic): boolean {
  return !!row.seedId && !!row.entityId;
}

export function isEntity(row: Statistic, allRows: Statistic[]): boolean {
  return !isDepartment(row) && (row.seedId === undefined ||
    allRows
      .map((statistic: Statistic) => statistic.entityId === row.entityId)
      .filter((bool: boolean) => bool).length === 1);
}

export function isDepartment(row: Statistic): boolean {
  return row.entityId === undefined && row.seedId === undefined;
}
    ...
    this.dataSource.filterPredicate = (data: Statistic, filter: string): boolean => {
      switch (filter) {
        case Filter.Seed:
          return isSeed(data);
        case Filter.Entity:
          return isEntity(data, this.dataSource.data);
        case Filter.Department:
          return isDepartment(data);
      }
    };
  }
